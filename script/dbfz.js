...
const generateSrc = (character, rendersDir, imgExt) =>
  `./${rendersDir}/${character.renderName}.${imgExt}`;
...
const charIndex = (characterZ, characterName) =>
  characterZ.map((character) => character.name).indexOf(characterName);

const returnIndex = (characterZ, dataset, action) => {
  let index = eval(
    `${charIndex(
      characterZ,
      $(`img[data-position=${dataset.position}]`).dataset.char
    )} ${action[dataset.direction]} 1`
  );
...
const swapData = (character, pos, act) => {
  ...
  [newCharName.dataset.char, character.dataset.char] = [
    character.dataset.char,
    newCharName.dataset.char,
  ];
  [newCharName.dataset.assist, character.dataset.assist] = [
    character.dataset.assist,
    newCharName.dataset.assist,
  ];
  changeData(
    character,
    generateSrc(
      characterZ[charIndex(characterZ, character.dataset.char)],
      "renderZ",
      "png"
    ),
    characterZ[charIndex(characterZ, character.dataset.char)]
  ...
  changeData(
    newCharName,
    generateSrc(
      characterZ[charIndex(characterZ, newCharName.dataset.char)],
      "renderZ",
      "png"
    ),
    characterZ[charIndex(characterZ, newCharName.dataset.char)]
...
const changeAssist = (character, pos, act) => {
  ...
  const assistType = Object.keys(assist).filter(
    (key) => assist[key] === newAssist
  );
  character.dataset.assist = assistType;
  assistField.textContent =
    characterZ[charIndex(characterZ, character.dataset.char)].zAssist[
      assistType
    ];
...
$$(".arrow").forEach((arrow) => {
  arrow.addEventListener("click", (e) =>
    changeRank(rankImg.dataset.rank, e.currentTarget.dataset.direction)
  );
});
$$(".Zarrow").forEach((arrow) => {
  arrow.addEventListener("click", (e) => {
    ...
    dataset.action === "Zchange" &&
      changeData(
        $(`img[data-position=${dataset.position}]`),
        generateSrc(characterZ[index], "renderZ", "png"),
        characterZ[index]
      );
    dataset.action === "Zswitch" &&
      swapData(
        $(`img[data-position=${dataset.position}]`),
        dataset.position,
        dataset.direction
      );
    dataset.action === "Zassist" &&
      changeAssist(
        $(`img[data-position=${dataset.position}]`),
        dataset.position,
        dataset.direction
...
$$(".colors > div").forEach((color) => {
  color.addEventListener("click", (e) => {
    ...
    const charColor =
      characterZ[
        charIndex(characterZ, $(`img[data-position=${className}]`).dataset.char)
      ].color;
    const newColor = adjust(charColor, random(80));
    $(
      `.charContainer[data-position=${className}]`
...
document.addEventListener("dblclick", () => {
  if ( $("body").classList.contains("photoMode")) {
     $("body").classList.remove("photoMode");
     $(".characterContainer").classList.remove("hideAssists");
     $(".gamer").classList.remove("hideGamerTag");
     $(".rankBig").classList.remove("hideRank");
...
$(".generate").addEventListener("click", (e) => {
  ...
  $$("img[data-position]").forEach((img, i) => {
    changeData(
      img,
      generateSrc(characterZ[randoms[img.dataset.position]], "renderZ", "png"),
      characterZ[randoms[img.dataset.position]]
    );
    $(".headerLogo  img").classList.add("loading");
...
$(".link").addEventListener("click", () => {
  let chars = [];
  $$("img[data-position]").forEach((img, i) => {
    chars[i] = `${img.dataset.position[0]}=${charIndex(
      characterZ,
      img.dataset.char
    )}-${img.dataset.assist}`;
  });
...
$$(".orientation > div").forEach((skew) =>
  skew.addEventListener("click", (e) => {
...
$$(".platform > div").forEach((platform) =>
  platform.addEventListener("click", (e) => {
    if (e.currentTarget.classList.contains("selectedPlatform")) {
      ...
      $$(".platform > div").forEach((div) =>
        div.classList.remove("selectedPlatform")
      );
...
const initializeData = () => {
  ...
  let total = Array.from(charZ)
    .map(
      (character) =>
        tierZ[characterZ[charIndex(characterZ, character.dataset.char)].tier]
    )
    .reduce((a, b) => a + b, 0);
  ...
  charContainer.forEach((container, index) => {
    let character =
      characterZ[charIndex(characterZ, charZ[index].dataset.char)];
  ...
  assists.forEach((assistName, index) => {
    ...
    assistName.textContent =
      characterZ[charIndex(characterZ, charName)].zAssist[assistType];
    if (charName === "Captain Ginyu" || charName === "Majin Buu")
      charZ[index].classList.add("smol");
  ...
  if (team[0] && team[1] && team[2]) {
    ...
    assists.forEach((assistName, index) => {
      ...
      changeData(
        ...
        generateSrc(characterZ[team[index][0]], "renderZ", "png"),
        characterZ[team[index][0]]
      );
      if (charName === "Captain Ginyu" || charName === "Majin Buu")
        charZ[index].classList.add("smol");
      else charZ[index].classList.remove("smol");
...
function random(n, p = undefined) {
  ...
  return p
    ...
    : Math.floor(Math.random() * n) * plusOrMinus;
}

function load() {
  let images = [];
  characterZ.forEach((char) =>
    images.push((new Image().src = generateSrc(char, "renderZ", "png")))
  );
}
